...
class CRSTB(nn.Module):
    ...
    def forward(self, x, y, x_size):
        ...
        x_HDC1 = self.hdc1(x_original)
        x_HDC2 = F.interpolate(x_HDC1, size=(int(np.sqrt(l)), int(np.sqrt(l))), mode='bilinear', align_corners=False)
        x_HDC2 = x_HDC2.permute(0, 2, 3, 1).contiguous()  # 重新排列维度为 [1, H, W, 60]
        b1, h1, w1, c1 = x_HDC2.shape
        x_HDC2 = x_HDC2.view(b1, h1 * w1, c1)

        y_original = y.view(b, int(np.sqrt(l)), int(np.sqrt(l)), c)
        y_original = y_original.permute(0, 3, 1, 2)

        y_HDC1 = self.hdc2(y_original)
        y_HDC2 = F.interpolate(y_HDC1, size=(int(np.sqrt(l)), int(np.sqrt(l))), mode='bilinear', align_corners=False)
        y_HDC2 = y_HDC2.permute(0, 2, 3, 1).contiguous()  # 重新排列维度为 [1, H, W, 60]
        b2, h2, w2, c2 = y_HDC2.shape
        y_HDC2 = y_HDC2.view(b2, h2 * w2, c2)
...
if __name__ == '__main__':
    ...
    model = SwinFusion(upscale=1, img_size=128,in_chans=1,
