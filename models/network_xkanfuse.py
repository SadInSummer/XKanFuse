# -----------------------------------------------------------------------------------
# SwinKansformer: Image Restoration Using Swin Kansformer based on Swin Transformer, https://arxiv.org/abs/2108.10257
...
class SwinKansformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        x = x + self.drop_path(self.kan(self.norm2(x).reshape(-1, x.shape[-1])).reshape(B, L, C))
...
class Cross_SwinKansformerBlock(nn.Module):
    r""" Swin Kansformer Block.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.kan_A = KAN([dim, 64, dim])
        self.kan_B = KAN([dim, 64, dim])
    ...
    def forward(self, x, y, x_size):
        ...
        x = x + self.drop_path_A(self.kan_A(self.norm2_A(x).reshape(-1, x.shape[-1])).reshape(B, L, C))

        y = shortcut_B + self.drop_path_B(y)
        # y = y + self.drop_path_B(self.mlp_B(self.norm2_B(y)))
        y = y + self.drop_path_B(self.kan_B(self.norm2_B(y).reshape(-1, y.shape[-1])).reshape(B, L, C))
        return x, y
...
class BasicLayer(nn.Module):
    """ A basic Swin Kansformer layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinKansformerBlock(dim=dim, input_resolution=input_resolution,
...
class Cross_BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            Cross_SwinKansformerBlock(dim=dim, input_resolution=input_resolution,
                                       num_heads=num_heads, window_size=window_size,
                                       shift_size=0 if (i % 2 == 0) else window_size // 2,
                                       mlp_ratio=mlp_ratio,
                                       qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class RSKB(nn.Module):
    """Residual Swin Kansformer Block (RSKB).
...
class CRSKB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=224, patch_size=4, resi_connection='1conv'):
        super(CRSKB, self).__init__()

        self.dim = dim
        self.input_resolution = input_resolution

        self.mdc1_A = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=60, out_channels=60, kernel_size=3, stride=1, padding=0, dilation=1),
            nn.BatchNorm2d(60),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(),
            # 第二层 (3-1)*2+1=5 （62-5)/1 + 1 =58
            nn.Conv2d(in_channels=60, out_channels=60, kernel_size=3, stride=1, padding=0, dilation=2),
            nn.BatchNorm2d(60),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(),
            # 第三层 (3-1)*5+1=11  (58-11)/1 +1=48
            nn.Conv2d(in_channels=60, out_channels=60, kernel_size=3, stride=1, padding=0, dilation=5),
            nn.BatchNorm2d(60),
            # inplace-选择是否进行覆盖运算
            nn.ReLU()
        )
        self.mdc2_A = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=60, out_channels=60, kernel_size=3, stride=1, padding=0, dilation=1),
            nn.BatchNorm2d(60),
            # inplace-选择是否进行覆盖运算
    ...
    def forward(self, x, y, x_size):
        ...
        b, l, c = x.shape
        x_original = x.view(b, int(np.sqrt(l)), int(np.sqrt(l)), c)
        x_original = x_original.permute(0, 3, 1, 2)

        x_MDC1 = self.mdc1_A(x_original)
        x_MDC1 = F.interpolate(x_MDC1, size=(int(np.sqrt(l)), int(np.sqrt(l))), mode='bilinear', align_corners=False)
        x_MDC1 = x_MDC1.permute(0, 2, 3, 1).contiguous()  # 重新排列维度为 [1, H, W, 60]
        b1, h1, w1, c1 = x_MDC1.shape
        x_MDC1 = x_MDC1.view(b1, h1 * w1, c1)

        x_MDC2 = self.mdc2_A(x_original)
        x_MDC2 = F.interpolate(x_MDC2, size=(int(np.sqrt(l)), int(np.sqrt(l))), mode='bilinear', align_corners=False)
        x_MDC2 = x_MDC2.permute(0, 2, 3, 1).contiguous()  # 重新排列维度为 [1, H, W, 60]
        b2, h2, w2, c2 = x_MDC2.shape
        x_MDC2 = x_MDC2.view(b2, h2 * w2, c2)

        y_original = y.view(b, int(np.sqrt(l)), int(np.sqrt(l)), c)
        y_original = y_original.permute(0, 3, 1, 2)

        y_MDC1 = self.mdc1_B(y_original)
        y_MDC1 = F.interpolate(y_MDC1, size=(int(np.sqrt(l)), int(np.sqrt(l))), mode='bilinear', align_corners=False)
        y_MDC1 = y_MDC1.permute(0, 2, 3, 1).contiguous()  # 重新排列维度为 [1, H, W, 60]
        b3, h3, w3, c3 = y_MDC1.shape
        y_MDC1 = y_MDC1.view(b3, h3 * w3, c3)

        y_MDC2 = self.mdc2_B(y_original)
        y_MDC2 = F.interpolate(y_MDC2, size=(int(np.sqrt(l)), int(np.sqrt(l))), mode='bilinear', align_corners=False)
        y_MDC2 = y_MDC2.permute(0, 2, 3, 1).contiguous()  # 重新排列维度为 [1, H, W, 60]
        b4, h4, w4, c4 = y_MDC2.shape
        y_MDC2 = y_MDC2.view(b4, h4 * w4, c4)
        ...
        x2, y2 = self.residual_group(x1, y1, x_size)
...
class AttentionWise(nn.Conv2d):

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=1, dilation=1, groups=1, bias=True):
        super(AttentionWise, self).__init__(in_channels, out_channels, kernel_size, stride,
    ...
    def forward(self, x):
        if self.ind:
            ...
            out_self_att = out_self_att.view(b, self.oc, h, w)
...
class XKanFuse(nn.Module):
    r""" SwinKansformer
        A PyTorch impl of : `SwinKansformer: Image Restoration Using Swin Kansformer`, based on Swin Transformer.
    ...
    Args:
        ...
        depths (tuple(int)): Depth of each Swin Kansformer layer.
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
        ...
        self.ckan_first1_A = KAN_Convolutional_Layer(n_convs=5, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),
                                                     device=device)
        self.ckan_first2_A = KAN_Convolutional_Layer(n_convs=12, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),
                                                     device=device)
        self.attention1_A = AttentionWise(1, 5)
        self.attention2_A = AttentionWise(5, 60)

        self.ckan_first1_B = KAN_Convolutional_Layer(n_convs=5, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),
                                                     device=device)
        self.ckan_first2_B = KAN_Convolutional_Layer(n_convs=12, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),
                                                     device=device)
        self.attention1_B = AttentionWise(1, 5)
        self.attention2_B = AttentionWise(5, 60)
    ...
    def forward_features_Ex_A(self, x):
        x1 = self.ckan_first1_A(x)
        x2 = self.attention1_A(x)
        x = x1 + x2
        x3 = self.ckan_first2_A(x)
        x4 = self.attention2_A(x)
        x = x3 + x4
    ...
    def forward_features_Ex_B(self, x):
        x1 = self.ckan_first1_B(x)
        x2 = self.attention1_B(x)
        x = x1 + x2
        x3 = self.ckan_first2_B(x)
        x4 = self.attention2_B(x)
        x = x3 + x4
...
if __name__ == '__main__':
    ...
    model = XKanFuse(upscale=1, img_size=128,in_chans=1,
                       window_size=8, img_range=1., depths=[1, 1, 1, 1],
                       embed_dim=60, num_heads=[1, 1, 1, 1], mlp_ratio=2)
